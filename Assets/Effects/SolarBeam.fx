...
float4 Beam(float2 coords : TEXCOORD0) : COLOR0
{
    ...
    if (dot(p2 - p1, p - p1) > 0.0 && dot(p1 - p2, p - p2) > 0.0)
    {
        ...
        dist = min(dist, length(a - dot(a, b) * b) + indent*2);
    }
    
    float4 edgeCol = float4(255, 220, 100, 255) / 255.0;
    
    // Output to screen
    if (dist < thickness)
    {
        return lerp(float4(1, 1, 1, 1), edgeCol, pow(dist / thickness, cos(myParam*5.0)+5.0));
    }
    else
    {
